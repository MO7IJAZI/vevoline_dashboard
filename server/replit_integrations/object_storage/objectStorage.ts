...
export class ObjectStorageService {
  ...
  async getObjectEntityUploadURL(): Promise<string> {
    ...
    // Logic check already handled in getPrivateObjectDir but explicit check here for clarity
    if (!privateObjectDir && IS_REPLIT) {
       // Should not happen as getPrivateObjectDir throws
    }
  ...
  async getObjectEntityFile(objectPath: string): Promise<File | LocalFile> {
    ...
    if (!IS_REPLIT) {
        ...
        const file = new LocalFile(objectEntityPath);
        const [exists] = await file.exists();
        if (!exists) {
            throw new ObjectNotFoundError();
        }
  ...
  normalizeObjectEntityPath(
    ...
    if (!IS_REPLIT) {
      if (rawPath.startsWith("/api/local-upload/")) {
          const entityId = rawPath.replace("/api/local-upload/", "");
